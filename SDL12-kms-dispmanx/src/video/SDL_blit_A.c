...
static void BlitRGBtoRGBPixelAlpha(SDL_BlitInfo *info)
{
	...
	while(height--) { 
		...
		/* FIXME: Here we special-case opaque alpha since the
		   ...
		   it correctly. Also special-case alpha=0 for speed?
		   Benchmark this! */
		//MAC dstp NO es igual a current->pixels de SDL_fbvideo.c, sino que es un puntero
		//a UNA PARTE de ese array de pixels, porque esta función de blitting copia un 
		//rectángulo a otro rectángulo, y no la pantalla entera. Si lo piensas, tienes un 
		//área de 320x200 en un display de 1280x800. 
		//Sin embargo, la variable de la que se deriva dstp sí que coincide con curr->pixels.
		if(alpha) {   
		  if(alpha == SDL_ALPHA_OPAQUE) {
		    *dstp = s; 
		    //MAC optimización mágica
	            //*dstp = (s & 0x00ffffff) | (*dstp & 0xff000000);  
		} else {
		    /*
