 ...
 */
static void KMS_vgainitkeymaps(int fd)
{
...
int KMS_InGraphicsMode(_THIS)
{
	return((keyboard_fd >= 0) && (saved_kbd_mode >= 0));
}

int KMS_EnterGraphicsMode(_THIS)
{
	struct termios keyboard_termios;

	// Set medium-raw keyboard mode 
	if ( (keyboard_fd >= 0) && !KMS_InGraphicsMode(this) ) {
...
void KMS_LeaveGraphicsMode(_THIS)
{
	if ( KMS_InGraphicsMode(this) ) {
...
void KMS_CloseKeyboard(_THIS)
{
	if ( keyboard_fd >= 0 ) {
		KMS_LeaveGraphicsMode(this);
		if ( keyboard_fd > 0 ) {
			close(keyboard_fd);
		}
	}
...
int KMS_OpenKeyboard(_THIS)
{
	/* Open only if not already opened */
 	if ( keyboard_fd < 0 ) {
		...
		/* Set up keymap */
		KMS_vgainitkeymaps(keyboard_fd);
 	}
 	return(keyboard_fd);
}
...
void KMS_vgamousecallback(int button, int relative, int dx, int dy)
{
...
static void handle_tslib(_THIS)
{
	...
	while (ts_read(ts_dev, &sample, 1) > 0) {
		...
		KMS_vgamousecallback(button, 0, sample.x, sample.y);
	}
	return;
}
#endif /* SDL_INPUT_TSLIB */
...
static void handle_mouse(_THIS)
{
	...
	for ( i=0; i<(nread-(packetsize-1)); i += packetsize ) {
			...
			case MOUSE_IMPS2:
				...
				switch (mousebuf[i+3]&0x0F) {
				    ...
				    case 0x0F: /* DY = +1 (map button 4) */
                                       KMS_vgamousecallback(button | (1<<3),
                                                           1, 0, 0);
					break;
				    case 0x01: /* DY = -1 (map button 5) */
                                       KMS_vgamousecallback(button | (1<<4),
                                                           1, 0, 0);
					break;
				}
				break;
		...
		KMS_vgamousecallback(button, relative, dx, dy);
	}
	if ( i < nread ) {
...
static void switch_vt_prep(_THIS)
{
	...
	//KMS_SavePaletteTo(this, 256, screen_palette);
...
static void switch_vt_done(_THIS)
{
	...
	//KMS_RestorePaletteFrom(this, 256, screen_palette);
	if ( screen_contents ) {
