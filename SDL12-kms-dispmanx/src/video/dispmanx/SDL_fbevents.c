 ...
 */
static void DISPMANX_vgainitkeymaps(int fd)
{
...
int DISPMANX_InGraphicsMode(_THIS)
{
	return((keyboard_fd >= 0) && (saved_kbd_mode >= 0));
}

int DISPMANX_EnterGraphicsMode(_THIS)
{
	struct termios keyboard_termios;

	// Set medium-raw keyboard mode 
	if ( (keyboard_fd >= 0) && !DISPMANX_InGraphicsMode(this) ) {
...
void DISPMANX_LeaveGraphicsMode(_THIS)
{
	if ( DISPMANX_InGraphicsMode(this) ) {
...
void DISPMANX_CloseKeyboard(_THIS)
{
	if ( keyboard_fd >= 0 ) {
		DISPMANX_LeaveGraphicsMode(this);
		if ( keyboard_fd > 0 ) {
			close(keyboard_fd);
		}
	}
...
int DISPMANX_OpenKeyboard(_THIS)
{
	/* Open only if not already opened */
 	if ( keyboard_fd < 0 ) {
		...
		/* Set up keymap */
		DISPMANX_vgainitkeymaps(keyboard_fd);
 	}
 	return(keyboard_fd);
}
...
void DISPMANX_CloseMouse(_THIS)
{
#if SDL_INPUT_TSLIB
	if (ts_dev != NULL) {
		ts_close(ts_dev);
...
void DISPMANX_vgamousecallback(int button, int relative, int dx, int dy)
{
...
static void handle_tslib(_THIS)
{
	...
	while (ts_read(ts_dev, &sample, 1) > 0) {
		...
		DISPMANX_vgamousecallback(button, 0, sample.x, sample.y);
	}
	return;
}
#endif /* SDL_INPUT_TSLIB */
...
static void handle_mouse(_THIS)
{
	...
	for ( i=0; i<(nread-(packetsize-1)); i += packetsize ) {
			...
			case MOUSE_IMPS2:
				...
				switch (mousebuf[i+3]&0x0F) {
				    ...
				    case 0x0F: /* DY = +1 (map button 4) */
                                       DISPMANX_vgamousecallback(button | (1<<3),
                                                           1, 0, 0);
					break;
				    case 0x01: /* DY = -1 (map button 5) */
                                       DISPMANX_vgamousecallback(button | (1<<4),
                                                           1, 0, 0);
					break;
				}
				break;
		...
		DISPMANX_vgamousecallback(button, relative, dx, dy);
	}
	if ( i < nread ) {
...
static void switch_vt_prep(_THIS)
{
	...
	//MAC No recupero nada. No podré cambiar de VT, supongo.
	//DISPMANX_SavePaletteTo(this, 256, screen_palette);
...
static void switch_vt_done(_THIS)
{
	...
	//DISPMANX_RestorePaletteFrom(this, 256, screen_palette);
	if ( screen_contents ) {
...
void DISPMANX_PumpEvents(_THIS)
{
	fd_set fdset;
	int max_fd;
...
void DISPMANX_InitOSKeymap(_THIS)
{
	int i;
